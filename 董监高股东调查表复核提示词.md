# 董监高股东调查表复核提示词（附 check_survey.py 脚本全文）

> **适用场景**：IPO/新三板申报项目中《自然人股东、董事、监事、高级管理人员及核心技术人员调查表》（一人一份 docx）的批量复核。当用户要求复核、核对、批量核查调查表，或提到报告期改口、留空未填、身份证/学历/工作经历核查、单位全称核查、对外任职投资核查、近亲属及其控制企业核查、亲属互填一致性时使用。
> **使用方式**：本文既可直接作为提示词交给 Agent，也可作为完整 Agent Skill 安装（SKILL.md + scripts/check_survey.py，收录于 [licheng-skills](https://github.com/lennonli/licheng-skills) 仓库 `survey-form-review/`；附录含脚本全文，落盘即用）。
> **核心原则**：机械规则交给脚本，判断与核查交给模型，公开信息交给企查查/元典 MCP，股权数据以底稿为准。同一批次多份表格一起复核价值最大——夫妻、兄弟姐妹互填内容可以相互印证。
